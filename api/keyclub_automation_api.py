...
def fetch_docs_data(id, credentials): # only used for getting stuff from event sign up docs
    ...
    for table in doc_tables:
        ...
        for i in range(len(row[0].get("tableCells"))): # finds the cols for name, hours, sign in/out, and optionally grade and phone for each table
            ...
            match(col): # matches cols
                case "name": name_col = i
                case "hours": hours_col = i
                case "sign in": start_col = i
                case "sign out": end_col = i
                case "grade": grade_col = i
                case "email/ phone": phone_col = i
        ...
        for i in range(1, len(row)): # gets data for the rest of the rows, after the first header row
            if start_col != "" and end_col != "": # if sign in/out col are present (not donation event) then locate
                try:
                    start = row[i].get("tableCells")[start_col].get("content")[0].get("paragraph").get("elements")[0].get("textRun").get("content").replace("\n", "").lower()
                    end = row[i].get("tableCells")[end_col].get("content")[0].get("paragraph").get("elements")[0].get("textRun").get("content").replace("\n", "").lower()
                except:
                    pass
            else:
                start = ""
            ...
            grade = row[i].get("tableCells")[grade_col].get("content")[0].get("paragraph").get("elements")[0].get("startIndex") # get grade col
            phone = row[i].get("tableCells")[phone_col].get("content")[0].get("paragraph").get("elements")[0].get("startIndex") # get phone col
            name = row[i].get("tableCells")[name_col].get("content")[0].get("paragraph").get("elements")[0].get("textRun").get("content").replace("\n", "").lower() # get name col
            hours = row[i].get("tableCells")[hours_col].get("content")[0].get("paragraph").get("elements")[0].get("textRun").get("content").replace("\n", "").lower() # get hour col
            
            if name != "" and start != "" and end != "":
                if hours == "": # if no hours, save index
                    hours = row[i].get("tableCells")[hours_col].get("content")[0].get("paragraph").get("elements")[0].get("startIndex")
                volunteers.update({name: {"hours": hours, "start": start, "end": end}})
            elif name == "":
                name = row[i].get("tableCells")[name_col].get("content")[0].get("paragraph").get("elements")[0].get("startIndex")
...
def log_event(id, hours_multiplier, credentials, **kwargs):
    ...
    if kwargs.get("meeting_length"): # if meetin_length given, assume it is a meeting, otherwise it is an event
        ...
        if not event_volunteers: # if empty event end program
        ...
        elif isinstance(event_volunteers.get(list(event_volunteers)[0]).get("hours"), numbers.Number): # if first volunteer doesnt have hours filled out, assume all other volunteers dont either
            ...
            for name in event_volunteers:
                ...
                hours = str(round((((end[0] * 60 + end[1]) - (start[0] * 60 + start[1])) / 60) * hours_multiplier, 2)) # calculates hours
    ...
    all_rows = fetch_sheet_data(id=hours_spreadsheet_id, ranges=[hours_spreadsheet_range[0], hours_spreadsheet_range[1]], credentials=credentials).get("data") # list of all volunteer names
    ...
    for name in event_volunteers:
        try:
            ...
            volunteer_values.append(float(event_volunteers.get(name).get("hours")) * hours_multiplier) # adds values to write
            volunteer_logged.update({name: {"hours": float(event_volunteers.get(name).get("hours")) * hours_multiplier}}) # logs volunteers logged
        except:
...
# note to self: finish combining names function on line 170
...
#     month = month.lower()
#     form_responses = fetch_sheet_data(form_spreadsheet_id, spreadsheet_range, credentials).get("data")
